sap.ui.define([
    ...
    return Controller.extend("zautodesignapp.controller.zcount.transaction.zcountbasecontroller", {
        onInit: function () {
            ...
            this.zcountmodel = new JSONModel({
                HeaderData: [{
                    processno: "",
            ...
            this.getView().setModel(this.zcountmodel, "zcountmodel");

            this.TabZcountItemModel = new sap.ui.model.json.JSONModel({
                DatasZitem: [],
            });
        ...
        //     var sProcessOrders = oView.byId("idprocessorder").getValue();
        ...
        onGoItemPages: function () {
            ...
            var oScreenModel = this.screenModel;
            var oZCountModel = oView.getModel("zcountmodel");
            var oItemDataModel = oView.getModel("TabZcountItemModel");
            ...
            oODataModel.read("/ZCE_ZCOUNT_HEAD", {
                ...
                success: function (oData) {
                    ...
                    if (aResults.length > 0) {
                        ...
                        // Set header data
                        oZCountModel.setProperty("/HeaderData/0", aResults[0]);
                        oZCountModel.updateBindings(true);
        ...
        //     // Normalize process order formats
        //     var processOrderLegacy = "00" + rawProcessOrder;
            ...
            if (tabledata.length > 0) {
                ...
                oDataModel.read("/ZCE_ZCOUNT_ITEM", {
                    ...
                    success: function (oData) {
                        ...
                        //     sap.m.MessageToast.show("No more data available.");
                        //     return;
                        // }
                        ...
                        // Calculate lastBoxNo and lastSrno from backend data
                        var lastBoxNo = backendItems.reduce((max, item) => {
                            var val = parseInt(item.boxno);
                            return isNaN(val) ? max : Math.max(max, val);
                        }, 0);
                        ...
                        // Add new item to the table data
                        tabledata.push(newItem);
                        oModel.setProperty("/DatasZitem", tabledata);
                        oModel.refresh();
        ...
        onchangeavgwt: function (oEvent) {
            ...
            var context = input.getBindingContext("TabZcountItemModel");
            ...
            if (isValid && targetQty > 0) {
                ...
                this.zcountmodel.setProperty("/HeaderData/0/counted_qty", updatedcounted_qty.toFixed(2));

                // Set updated target_qty in header
                this.zcountmodel.setProperty("/HeaderData/0/target_qty", updatedHeaderTargetQty.toFixed(3));
        ...
        OnRowDelete: function (oEvent) {
            ...
            const mod = this.getView().getModel("TabZcountItemModel");
            const headerModel = this.zcountmodel;
...
`;
            ...
            sap.m.MessageBox.warning("Do you want to submit this data?", {
                ...
                onClose: async function (sAction) {
                    ...
                    try {
                        ...
                        const getHeaderData = that.zcountmodel.getProperty("/HeaderData/");
                        // const getItemData = that.TabZcountItemModel.getProperty("/DatasZitem");

                        const oItemModel = that.getView().getModel("TabZcountItemModel");
                        const getItemData = oItemModel ? oItemModel.getProperty("/DatasZitem") : [];
                        const headerdata = that.getView().getModel("ZCE_ZDTCOUNT_HEAD_SRVB");
                        const itemdata = that.getView().getModel("ZSB_ZCOUNT_ITEM");
                        ...
                        //const rawBatch = header.Batch ? header.Batch.toString() : "";
                        const processOrderFinal = rawProcessOrder.padStart(12, "0").slice(-12);  // max length 12
                        //const batchFinal = rawBatch.padStart(10, "0").slice(-10);
                        const batchFinal = header.Batch

                        const generatedUUID = processOrderFinal + batchFinal
                        ...
                        if (!sap_uuid) {
                            // --- Create new header ---
                            oHeaderEntry = {
                                // sap_uuid: "00" + header.ProcessOrder,
                                // id: "00" + header.ProcessOrder,
                                batch: header.Batch,
                                // process_order: "00" + header.ProcessOrder,
                            ...
                            await new Promise((resolve, reject) => {
                                headerdata.create("/ZCE_ZDTCOUNT_HEAD", oHeaderEntry, {
                                    success: function (oDATA) {
                        ...
                        for (const item of getItemData) {
                            ...
                            const Itempayload = {
                                ...
                                sap_uuid: item_uuid,
                                srno: item.srno,
                                //process_order: "00" + item.process_order,
                            ...
                            await new Promise((resolve, reject) => {
                                itemdata.create("/ZCE_ZCOUNT_ITEM", Itempayload, {
                                    success: function (oDATA) {
        ...
        onTarewt: function () {
            ...
            if (!oModel) {
                console.error("Model 'ZSB_ZCOUNT_ITEM' is not defined.");
                sap.m.MessageToast.show("Data model not found.");
                return;
            }
        ...
        onTarewtupate: async function () {
            const oView = this.getView();
            const oModel = oView.getModel("ZSB_ZCOUNT_ITEM"); // OData model for items
            const oTableModel = oView.getModel("TabZcountItemModel");
            const aItems = oTableModel.getProperty("/DatasZitem");
